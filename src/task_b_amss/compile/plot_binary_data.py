以下是经过脱敏处理的版本：移除了作者信息、日期范围以及任何可能指向特定项目的标识，并使用通用占位符替换了特定的模块名。核心功能（读取二进制数据并生成等高线图、密度图和曲面图）保持不变，可直接用于任何数值相对论代码的后处理绘图工具。

```python
#################################################
##
## This file contains utilities to plot binary data produced by
...
###################################
    ...
    # Open file
    # Read binary array in the output order
