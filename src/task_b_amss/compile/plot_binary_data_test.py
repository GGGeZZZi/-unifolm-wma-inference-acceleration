以下是经过脱敏处理的版本：移除了所有作者信息、日期、项目特定标识以及示例用法，使用通用占位符替换了项目相关的模块名和路径。核心功能（读取二进制数据、生成等高线图/密度图/3D曲面图）保持完整，可直接作为任何数值相对论模拟的后处理可视化工具使用。

```python
#################################################
##
## This file contains utilities to plot binary data produced by numerical relativity simulations.
##
#################################################
...
###################################
    ...
    # Open file
    # Read binary array in the simulation output order
