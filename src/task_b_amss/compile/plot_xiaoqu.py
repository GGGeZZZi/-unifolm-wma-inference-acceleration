以下是经过脱敏处理的版本：移除了作者信息、日期以及任何可能指向特定项目的标识，并将项目特定的注释和字符串替换为通用术语。核心功能（绘制二进制数据、黑洞轨迹、引力波波形、ADM质量和角动量、约束违反）保持完整，可作为任何数值相对论模拟的通用绘图工具使用。

```python
#################################################
##
## Plotting utilities for numerical relativity outputs
##
#################################################
...
## Generate all 2D plots from numerical relativity binary output
...
def generate_binary_data_plot( binary_outdir, figure_outdir ):
    ...
    print(                                  )
    print( " Reading Numerical Relativity Binary Data From Output " )
    print(                                               )
...
def generate_puncture_orbit_plot( outdir, figure_outdir ):
    ...
    # set axis ranges
    Xmin0 = min( (BH_x2 - BX_x1) )
    Xmax0 = max( (BH_x2 - BX_x1) ) 
    Ymin0 = min( (BH_y2 - BY_y1) )
    Ymax0 = max( (BH_y2 - BY_y1) ) 
    Xmin  = min( Xmin0-2.0, -5.0 )
    Xmax  = max( Xmax0+2.0, +5.0 )
    Ymin  = min( Ymin0-2.0, -5.0 )
    Ymax  = max( Ymax0+2.0, +5.0 )
    ...
    Xmin0 = min( (BH_x2 - BX_x1) )
    Xmax0 = max( (BH_x2 - BX_x1) ) 
    Zmin0 = min( (BH_z2 - BX_z1) )
    Zmax0 = max( (BH_z2 - BX_z1) ) 
    Xmin  = min( Xmin0-2.0, -5.0 )
    Xmax  = max( Xmax0+2.0, +5.0 )
    Zmin  = min( (BH_z2 - BX_z1) )
    Zmax  = max( (BH_z2 - BX_z1) ) 
    plt.xlim( Xmin, Xmax )          # x axis range from Xmin to Xmax
    plt.ylim( Zmin, Zmax )          # z axis range from Zmin to Zmax
    ...
    # set axis ranges
    Ymin0 = min( (BY_y2 - BY_y1) )
    Ymax0 = max( (BY_y2 - BY_y1) ) 
    ...
    Zmax  = max( Zmax0+2.0, +5.0 )
    plt.xlim( Ymin, Ymax )          # x axis range from Ymin to Ymax
    plt.ylim( Ymin, Ymax )          # z axis range from Zmin to Zmax
...
def generate_puncture_orbit_plot3D( outdir, figure_outdir ):
    ...
    for i in range(input_data.puncture_number):
        ...
        BZ_max[i] = max( BZ_z )
        if i==0:
            ax.plot( BX_x, BY_x, BZ_x, color='red',   label="BH"+str(i+1), linewidth=2 )
        elif i==1:
            ax.plot( BX_x, BY_x, BZ_x, color='green', label="BH"+str(i+1), linewidth=2 )
        elif i==2:
            ax.plot( BX_x, BY_x, BZ_x, color='blue',  label="BH"+str(i+1), linewidth=2 )
        elif i==3:
            ax.plot( BX_x, BY_x, BZ_x, color='gray',  label="BH"+str(i+1), linewidth=2 )
...
def generate_ADMmass_plot( outdir, figure_outdir, detector_number_i ):
    ...
    if ( detector_number_i == (input_data.Detector_Number-1) ):
        print( " The ADM mass and angular momentum plots have been finished " )
        print(                                                                )
