...
## Version: 1.0
## Note: Desensitized version. No machine-specific paths, sensitive info, or proprietary data.
##       All paths are relative or configurable, complying with data security requirements.
##
#################################################
...
File_directory   = "Test_Binary_System"          ## output file directory
...
Final_Evolution_Time     = 1.0                   ## final evolution time t1
Check_Time               = 100.0
Dump_Time                = 100.0                 ## time interval dT for dumping binary data
...
largest_box_xyz_max = [320.0, 320.0, 320.0]      ## scale of the largest box
                                                 ## not necessary to be cubic for "Patch" grid structure
...
quarter_sphere_number = 96                       ## grid number of 1/4 spherical surface
...
parameter_BH[0] = [ 36.0/(36.0+29.0+29),  0.0,  +0.31 ]   
...
## initial momentum for each puncture
...
Detector_Number = 12                           ## number of detector
Detector_Rmin   = 50.0                         ## nearest detector distance
Detector_Rmax   = 160.0                        ## farthest detector distance

#################################################
## Setting the apparent horizon
...
FR_Choice = 2                                  ## Choice options: 1, 2, 3, 4, 5
                                               ...
                                               ## 3: Schrodinger-Newton given by system phi(r) 
                                               ##    V(r)   = Exp(-8*Sqrt(PI/3)*phi(r)) * (1-Exp(4*Sqrt(PI/3)*phi(r)))**2 / (32*PI*a2)
